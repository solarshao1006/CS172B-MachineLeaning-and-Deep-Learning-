...
def get_args():
    ...
    parser.add_argument('-m', '--model', type=int, choices=[11,13,16,19], default=11, help="choose the depth of vgg" )
...
def test(opt):
    ...
    testdata, testGenerator, classes = preprocess(path=opt.test, batchsize=opt.batchsize,
                                                    imagesize=opt.imagesize , shuffle=False)
    images = [i[0] for i in testGenerator.sampler.data_source.imgs]
    print(images)
    ...
    if opt.conv1_1 and opt.model:
        ...
        model = torch.load(os.path.join('./models','VdcnnIR_C11_' + str(opt.model)))
    else:

        model = torch.load(os.path.join('./models','VdcnnIR_' + str(opt.model)))
    ...
    for idx_t, data_t in enumerate(testGenerator):
        ...
        with torch.no_grad():
            ...
            v.extend(_.cpu())
    print(metrics.accuracy_score(v,  total_testpredictions ))
...
if __name__ == '__main__':
    ...
    if os.path.exists('results/test_predictions.csv'):
        ...
        if len(df_old.columns) == (num_models):
        ...
        for i, image in enumerate(images):
            axes[i].imshow(img.imread('test/{}'.format(image)))
