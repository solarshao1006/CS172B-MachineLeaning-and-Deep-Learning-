...
def get_args():
    ...
    parser.add_argument('-lr', '--lr', type=float, default=0.001, help="learning rate for an Adam optimizer")
    args = parser.parse_args()
    return args

def train(opt):

    traindata, trainGenerator, classes = preprocess(path=opt.train, batchsize=opt.batchsize,
                                                    imagesize=opt.imagesize, shuffle=True)
    valdata, validationGenerator, classes = preprocess(path=opt.val, batchsize=opt.batchsize,
                                                      imagesize=opt.imagesize, shuffle=True)
    ...
    def plot_fig(train_loss, val_loss, accu = False):
        ...
        if accu:
            ...
            plt.ylabel("Accuracy")
            plt.legend()
            plt.savefig("figures/trainVal_accu_{}.png".format(opt.depth))
            plt.close()
            return
        if opt.conv1_1 and opt.depth==16:
    ...
    for epoch in range(opt.epochs):
        ...
        time_s= t1_stop-t1_start
        overall_time += time_s
        print("Training Epoch: {}\t , Time:{}".format(epoch+1,time_s))
        ...
        plot_fig(metrics.accuracy_score(total_labels,total_predictions), metrics.accuracy_score(total_ValLabels,total_Valpredictions), accu = True)
        # print(loss_fig)
    ...
    torch.save(model, 'models/VdcnnIR_final{}'.format(opt.depth))
    if opt.conv1_1 and opt.depth == 16:
    ...
    with open('results/Time.txt', 'a') as f:
        f.write('Model: {} Epoch: {}\t  Accuracy: {}\t Time: {}\n'.format(opt.depth, epoch,
                                                         metrics.accuracy_score(total_labels,total_predictions),overall_time))
